...
export function Globe(props: WorldProps) {
  ...
  const [countries, setCountries] = useState<{ features: Array<Record<string, unknown>> } | null>(null);
  ...
  useEffect(() => {
    if (groupRef.current && globeRef.current && props.activeLocation && props.locations) {
      ...
      if (targetLoc) {
        ...
        const camLat = Math.asin(camPos.y / camPos.length());

        const targetY = camLng - (targetLoc.lng * Math.PI / 180) - Math.PI / 2;
        const targetX = (targetLoc.lat * Math.PI / 180) - camLat;

        // Animate Y (Longitude) with shortest path
        ...
        import("framer-motion").then(({ animate }) => {
          ...
          animate(group.rotation.x, targetX, {
            type: "spring",
            stiffness: 120,
            damping: 20,
            mass: 1,
            onUpdate: (v) => { group.rotation.x = v; }
          });
        });
      }
    }
    ...
    >
      <div
        ...
        style={{
          ...
          opacity: activeLocation === loc.name ? 1 : 0.4,
  ...
  return (
    <Canvas camera={{ position: [0, 0, cameraZ], fov: 45, near: 1, far: 2000 }}>
      ...
      <OrbitControls
        ...
        maxDistance={cameraZ}
        autoRotateSpeed={1}
        autoRotate={!props.activeLocation}
